...
IT_PROTECTED_REMOTE_REPO_URL = os.getenv(
    "IT_PROTECTED_REMOTE_REPO", "https://dev.renku.ch/gitlab/contact/protected-renku.git"
)

IT_REMOTE_REPO_URL = os.getenv("IT_REMOTE_REPOSITORY", "https://dev.renku.ch/gitlab/contact/integration-test")
...
def dataset(client):
    ...
    with client.with_dataset("dataset", create=True) as dataset:
        dataset.creator = [Person(**{"affiliation": "xxx", "email": "me@example.com", "id": "me_id", "name": "me",})]
    return dataset
...
def svc_client_with_repo(svc_client_setup):
    ...
    svc_client.post("/cache.migrate", data=json.dumps(dict(project_id=project_id)), headers=headers)
