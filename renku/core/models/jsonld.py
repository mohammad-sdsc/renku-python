...
def attrs(maybe_cls=None, type=None, context=None, translate=None, **attrs_kwargs):
    ...
    def wrap(cls):
        ...
        if not issubclass(jsonld_cls, JSONLDMixin):
            jsonld_cls = attr.s(make_type(cls.__name__, (jsonld_cls, JSONLDMixin), {}), **attrs_kwargs)
        # Merge types
        ...
        property_context, scoped_properties = _add_class_property_contexts(jsonld_cls, context, cls, types)

        context.update(property_context)

        jsonld_cls.__module__ = cls.__module__
        jsonld_cls._jsonld_type = types[0] if len(types) == 1 else list(sorted(set(types)))
        ...
        jsonld_cls._jsonld_fields = {a.name for a in attr.fields(jsonld_cls) if KEY in a.metadata}

        context_doc = "\n".join("   " + line for line in json.dumps(context, indent=2).split("\n"))
        jsonld_cls.__doc__ = DOC_TPL.format(cls=cls, type=json.dumps(jsonld_cls._jsonld_type), context=context_doc,)

        # Register class for given JSON-LD @type
        try:
            type_ = pyld.jsonld.expand({"@type": jsonld_cls._jsonld_type, "@context": context})[0]["@type"]
        ...
        if type_ in jsonld_cls.__type_registry__ and str(jsonld_cls.__type_registry__[type_]) != str(jsonld_cls):
            raise TypeError(
                "Type {0!r} in {1!r} is already registered for {2!r}.".format(
                    jsonld_cls._jsonld_type, jsonld_cls, jsonld_cls.__type_registry__[type_],
                )
            )
...
def _add_class_property_contexts(jsonld_cls, context, original_cls, jsonld_ype):
...
def _propagate_reference_contexts(type_references, current_context, parent_context, parent_cls, parent_types):
    ...
    classes = [(c._jsonld_context, c._jsonld_type, c, True) for c in classes if hasattr(c, "_jsonld_context")]
    scoped_properties = False

    parent_cls_string = "{}.{}".format(parent_cls.__module__, parent_cls.__name__)
            ...
            for subtype in subtypes:
                ...
                if subcontext:
                    current_context["@context"].append(
                        {
                            fullname(cls) + "_" + subtype: {"@id": expanded_subtype, "@context": merge_ctx},
                            "@version": 1.1,
                        }
                    )
                else:
                    current_context["@context"].append(
                        {fullname(cls) + "_" + subtype: {"@id": expanded_subtype}, "@version": 1.1}
                    )
        ...
        if type_ in JSONLDMixin.__type_registry__:
            ...
            if all(str(t) not in str(new_cls) for t in types):
                raise NotImplementedError(
                    "Unexpected property type {},  expected one of {}".format(new_cls, ",".join(types))
                )
...
_container_types = (
    ("list", list, lambda type, value: [_default_converter(type, v) for v in value]),
    ("set", set, lambda type, value: {_default_converter(type, v) for v in value}),
    ("index", dict, lambda type, value: {k: _default_converter(type, v) for k, v in value.items()}),
)
...
def asjsonld(
    ...
    attrs = tuple(field for field in fields(inst.__class__) if field.name in jsonld_fields)
    rv = dict_factory()
    ...
    for a in attrs:
        ...
        if recurse is True:
            if has(v.__class__):
                ...
                rv[a.name] = df(
                    (
                        asjsonld(kk, dict_factory=df, add_context=False, basedir=basedir,)
                        if has(kk.__class__)
                        else convert_value(kk),
                        asjsonld(vv, dict_factory=df, add_context=False, basedir=basedir,) if has(vv.__class__) else vv,
                    )
    ...
    if inst_cls._jsonld_type:
        ...
        if use_scoped_type_form:
            rv_type = ["{}_{}".format(inst_cls._renku_type, t.replace(":", "_")) for t in rv_type]
...
class JSONLDMixin(ReferenceMixin):
    ...
    def from_jsonld(
        ...
        if "@type" in data:
            ...
            # registry, so we have to match it here.
            type_ = tuple(sorted(type_))
            if type_ in cls.__type_registry__ and getattr(cls, "_jsonld_type", None) != type_:
    ...
    def from_yaml(cls, path, client=None, commit=None):
        ...
        self = cls.from_jsonld(source, client=client, commit=commit, __reference__=path, __source__=deepcopy(source))
        return self
