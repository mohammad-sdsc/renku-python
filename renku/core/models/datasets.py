...
class CreatorMixin:
    ...
    creator = jsonld.container.list(Person, kw_only=True, context="schema:creator", converter=_convert_creators)
...
class Dataset(Entity, CreatorMixin):
    ...
    created = jsonld.ib(converter=parse_date, context="schema:dateCreated", kw_only=True)
    ...
    def data_dir(self):
        ...
        if self.client:
            return Path(self.client.data_dir) / self.short_name
        return ""
...
def is_dataset_short_name_valid(short_name):
    ...
    return short_name and LinkReference.check_ref_format(short_name, no_slashes=True) and "/" not in short_name
