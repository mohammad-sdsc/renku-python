...
def create_dataset(client, short_name, title=None, description="", creators=None, keywords=None, commit_message=None):
    ...
    dataset, _, __ = client.create_dataset(
        short_name=short_name, title=title, description=description, creators=creators, keywords=keywords
    )
...
def edit_dataset(client, short_name, title, description, creators, keywords=None, commit_message=None):
...
def _add_to_dataset(
    ...
    try:
        with client.with_dataset(short_name=short_name, create=create) as dataset:
...
def list_files(client, datasets=None, creators=None, include=None, exclude=None, format=None, columns=None):
    ...
    records = _filter(client, short_names=datasets, creators=creators, include=include, exclude=exclude)
...
def file_unlink(client, short_name, include, exclude, interactive=False, yes=False, commit_message=None):
    ...
    records = _filter(client, short_names=[short_name], include=include, exclude=exclude)
    if not records:
...
def dataset_remove(
    ...
    if not datasets:
        raise ParameterError("use dataset short_name or identifier", param_hint="short_names")

    unknown = [name for name, path in datasets.items() if not path or not path.exists()]
    if unknown:
        raise ParameterError("unknown datasets " + ", ".join(unknown), param_hint="short_names")
...
def import_dataset(
    client, uri, short_name="", extract=False, with_prompt=False, yes=False, commit_message=None, progress=None,
):
    ...
    if not provider.is_git_based:
        if not short_name:
            short_name = generate_default_short_name(dataset.name, dataset.version)
        ...
        if dataset.version:
            ...
            tag_dataset(client, short_name, tag_name, "Tag {} created by renku import".format(dataset.version))
    else:
...
def update_datasets(
    ...
    records = _filter(client, short_names=short_names, creators=creators, include=include, exclude=exclude)
...
def _filter(client, short_names=None, creators=None, include=None, exclude=None):
    ...
    for dataset in client.datasets.values():
        if not short_names or dataset.short_name in short_names:
            for file_ in dataset.files:
                ...
                if creators:
                    match = match and creators.issubset({creator.name for creator in file_.creator})
...
def tag_dataset_with_client(client, short_name, tag, description, force=False, commit_message=None):
