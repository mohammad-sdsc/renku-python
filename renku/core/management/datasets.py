...
from renku.core.models.datasets import Dataset, DatasetFile, DatasetTag, is_dataset_short_name_valid
...
class DatasetsApiMixin(object):
    ...
    def get_dataset_path(self, short_name):
        ...
        if not path.exists():
            try:
                path = LinkReference(client=self, name="datasets/" + short_name).reference
    ...
    def with_dataset(self, short_name=None, create=False):
        ...
        if dataset is None:
            ...
            dataset, path, dataset_ref = self.create_dataset(short_name=short_name)
        elif create:
            raise errors.DatasetExistsError('Dataset exists: "{}".'.format(short_name))
    ...
    def create_dataset(
        self, short_name=None, title=None, description=None, creators=None, keywords=None,
    ):
        """Create a dataset."""
        if not short_name:
        ...
        dataset_ref = LinkReference.create(client=self, name="datasets/" + short_name)
    ...
    def _add_from_local(self, dataset, path, external, destination):
        ...
        if src.is_dir():
            ...
            if path_in_repo:
                return [{"path": path_in_repo, "url": path_in_repo, "creator": dataset.creator, "parent": self}]
        ...
        for path, src, dst in files:
            if not src.is_dir():
                ...
                if based_on:
                    ...
                    based_on = DatasetFile.from_revision(remote_client, path=src, url=url, creator=creators)
    ...
    def add_dataset_tag(self, dataset, tag, description="", force=False):
        ...
        tag = DatasetTag(name=tag, description=description, commit=latest_commit.hexsha, dataset=dataset.name)
    ...
    def update_dataset_files(self, files, ref, delete=False):
        ...
        for file_ in updated_files:
            ...
            new_file = DatasetFile.from_revision(self, path=file_.path, based_on=file_.based_on, creator=creators)
