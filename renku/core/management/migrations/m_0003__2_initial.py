...
def _migrate_broken_dataset_paths(client):
    ...
    for dataset in client.datasets.values():
        ...
        expected_path = client.renku_datasets_path / Path(quote(dataset.identifier, safe=""))

        # migrate the refs
        ref = LinkReference.create(client=client, name="datasets/{0}".format(dataset.short_name), force=True,)
        ref.set_reference(expected_path / client.METADATA)

        if not dataset_path.exists():
            dataset_path = client.renku_datasets_path / uuid.UUID(dataset.identifier).hex
        ...
        for file_ in dataset.files:
            ...
            if not file_path.exists() and file_.path.startswith(".."):
                new_path = (client.renku_datasets_path / dataset.uid / file_path).resolve().relative_to(client.path)
                ...
                file_._id = urllib.parse.urljoin(
                    ...
                    posixpath.join("/blob/{hexsha}/{path}".format(hexsha=commit.hexsha, path=new_path)),
                )
    ...
    for dataset in client.datasets.values():
        for file_ in dataset.files:
            try:
                ...
                file_.commit = commit
                if not file_._label or "UNCOMMITTED" in file_._label or "@" not in file_._label:
