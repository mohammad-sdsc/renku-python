...
from renku.core.commands.dataset import add_file, create_dataset, edit_dataset, file_unlink, list_datasets, list_files
...
def add_file_to_dataset_view(user_data, cache):
    ...
    if not ctx["commit_message"]:
        ctx["commit_message"] = "service: dataset add {0}".format(ctx["short_name"])
...
def create_dataset_view(user, cache):
    ...
    try:
        ...
        return error_response(INTERNAL_FAILURE_ERROR_CODE, "push to remote failed silently - try again")
...
def edit_dataset_view(user_data, cache):
    ...
    if ctx.get("commit_message") is None:
        ctx["commit_message"] = "service: dataset edit {0}".format(ctx["short_name"])
...
def unlink_file_view(user_data, cache):
    ...
    if ctx.get("commit_message") is None:
        ...
        ctx["commit_message"] = "service: unlink dataset {0} {1}".format(ctx["short_name"], filters)
