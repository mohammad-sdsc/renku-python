...
def test_create_dataset_view(svc_client_with_repo):
    ...
    assert {"short_name", "remote_branch"} == set(response.json["result"].keys())
...
def test_create_dataset_with_metadata(svc_client_with_repo):
    ...
    assert response
    assert_rpc_response(response)
    ds = next(ds for ds in response.json["result"]["datasets"] if ds["short_name"] == payload["short_name"])
...
def test_create_dataset_view_unknown_param(svc_client_with_repo):
    ...
    payload = {"project_id": project_id, "short_name": "mydata", "remote_name": "origin"}
...
def test_add_file_view(svc_client_with_repo):
    ...
    assert {"short_name", "project_id", "files", "remote_branch"} == set(response.json["result"].keys())
...
def test_list_datasets_view(svc_client_with_repo):
    ...
    assert {"version", "description", "created_at", "short_name", "title", "creators", "keywords"} == set(
...
def test_create_and_list_datasets_view(svc_client_with_repo):
    ...
    assert {"datasets"} == set(response.json["result"].keys())
    assert 0 != len(response.json["result"]["datasets"])
    assert {"creators", "short_name", "version", "title", "description", "created_at", "keywords"} == set(
        response.json["result"]["datasets"][0].keys()
    )

    assert payload["short_name"] in [ds["short_name"] for ds in response.json["result"]["datasets"]]
...
def test_list_dataset_files(svc_client_with_repo):
    ...
    assert {"short_name", "files", "project_id", "remote_branch"} == set(response.json["result"].keys())
    assert file_id == response.json["result"]["files"][0]["file_id"]
...
def test_add_with_unpacked_archive(datapack_zip, svc_client_with_repo):
    ...
    payload = {"project_id": project_id, "short_name": payload["short_name"], "files": [{"file_id": file_["file_id"]},]}
...
def test_dataset_add_remote(url, svc_client_cache, project_metadata, mock_redis):
    ...
    assert_rpc_response(response)
    assert {"files", "short_name", "project_id"} == set(response.json["result"])
