...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "https://dev.renku.ch/projects/virginiafriedrich/datasets-test/" "datasets/b9f7b21b-8b00-42a2-976a-invalid",
            2,
        ),
...
def test_dataset_reimport_renku_dataset(runner, project):
    ...
    URL = (
        "https://dev.renku.ch/projects/virginiafriedrich/datasets-test/" "datasets/48299db3-8870-4cbe-a480-f75985c42a62"
    )
...
def test_import_from_renku_project(tmpdir, client, runner):
    ...
    file_ = read_dataset_file_metadata(remote_client, "zhbikes", "2019_verkehrszaehlungen_werte_fussgaenger_velo.csv")
...
def test_renku_clone_with_config(tmpdir):
    ...
    with chdir(str(tmpdir)):
        repo = project_clone(remote, config={"user.name": "sam", "user.email": "s@m.i", "filter.lfs.custom": "0"})
...
def test_renku_clone_checkout_rev(tmpdir):
    ...
    with chdir(str(tmpdir)):
        ...
        assert "3d387e64ea25079df8dd43b8875058cf9f4b0315" == str(repo.active_branch)
        reader = repo.config_reader()
        reader.values()
...
def test_renku_clone_checkout_revs(tmpdir, rev):
    ...
    with chdir(str(tmpdir)):
        repo = project_clone(remote, checkout_rev=rev,)

        assert rev == repo.active_branch.name


@pytest.mark.integration
@pytest.mark.parametrize("path,expected_path", [("", "datasets-test"), (".", "."), ("new-name", "new-name")])
@flaky(max_runs=10, min_passes=1)
