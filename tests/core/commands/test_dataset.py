...
def test_data_add_recursive(directory_tree, client):
    ...
    with client.with_dataset("dataset", create=True) as dataset:
        dataset.creator = [Person(name="me", email="me@example.com", id="me_id")]
...
def test_dataset_serialization(dataset):
    ...
    # check values
    assert str(dataset.created.isoformat()) == dataset_metadata.get("http://schema.org/dateCreated")
    assert dataset.creator[0].email == dataset_metadata.get("http://schema.org/creator")[0].get(
